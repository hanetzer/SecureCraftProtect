...
public class SCPTileEntity019 extends TileEntity
{
    ...
    public void updateEntity()
    {
        ...
        worldObj.spawnParticle("portal", d, d1, d2, 0.0D, 0.0D, 0.0D);
        worldObj.spawnParticle("portal", d, d1, d2, 0.0D, 0.0D, 0.0D);
        ...
        if (!worldObj.isRemote)
        {
            ...
            for (int i = 0; i < byte0; i++)
            {
                ...
                int j = worldObj.getEntitiesWithinAABB(entityliving.getClass(), AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1).expand(8D, 4D, 8D)).size();
