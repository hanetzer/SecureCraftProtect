...
public class SCPItem106 extends SCPItemDocument
{
		...
		else
		{
			...
			for(int height = -1; height <= 3; height++)
				for(int length = -1; length <= 6; length++)
					...
					{					
						par3World.setBlock(par4 + length, par5 + height, par6 + width, mod_SCP.CorrodedIron.blockID, 0, 2);					
					}
			...
			SCPItemCorrodedDoor.placeDoorBlock(par3World, par4 + 3, par5, par6 - 1, 1, mod_SCP.CorrodedDoor);

			par2EntityPlayer.addChatMessage("SCP-106 Spawned. | [Type: \u00a74Keter\u00a7f]");
	...
	{
		SCPEntity106 var8 = new SCPEntity106(par0World);
