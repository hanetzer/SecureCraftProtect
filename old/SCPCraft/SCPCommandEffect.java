...
public class SCPCommandEffect extends CommandEffect
{
    ...
    public void processCommand(ICommandSender par1ICommandSender, String[] par2ArrayOfStr)
    {
        if (par2ArrayOfStr.length >= 2)
        {
            ...
            if (i >= 0 && i < SCPPotion.potionTypes.length && SCPPotion.potionTypes[i] != null)
            {
                if (par2ArrayOfStr.length >= 3)
                {
                    k = parseIntBounded(par1ICommandSender, par2ArrayOfStr[2], 0, 1000000);

                    if (SCPPotion.potionTypes[i].isInstant())
                    {
                        j = k;
                    }
                    else
                ...
                else if (SCPPotion.potionTypes[i].isInstant())
                {
                    j = 1;
                }
                ...
                if (k == 0)
                {
                    ...
                    {
                        throw new CommandException("commands.effect.failure.notActive", new Object[] {StatCollector.translateToLocal(SCPPotion.potionTypes[i].getName()), entityplayermp.getEntityName()});
                    }

                    entityplayermp.removePotionEffect(i);
                    notifyAdmins(par1ICommandSender, "commands.effect.success.removed", new Object[] {StatCollector.translateToLocal(SCPPotion.potionTypes[i].getName()), entityplayermp.getEntityName()});
                }
                else
                {
