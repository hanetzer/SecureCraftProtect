...
public class SCPBlock789J extends Block
{
	...
	public static boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, float x, float y, float z, int north, int south, int east, int west)
	{
		if (!world.isRemote)
		{
			...
			entityplayer.attackEntityFrom(SCPDamageSource.ghostButt, 5);
			return true;
		}
		return true;
	}
	...
	{    	
		...
		if(l == 3) world.setBlockMetadataWithNotify(i, j, k, 0, 0); //~
		if(l == 2) world.setBlockMetadataWithNotify(i, j, k, 1, 0);
		if(l == 5) world.setBlockMetadataWithNotify(i, j, k, 2, 0);
		if(l == 4) world.setBlockMetadataWithNotify(i, j, k, 3, 0);
	}
	...
	{
		par1World.playSoundEffect((double)((float)par2 + 0.5F), (double)((float)par3 + 0.5F), (double)((float)par4 + 0.5F), "sounds.SCP789J", 2F, 1F);
	}
