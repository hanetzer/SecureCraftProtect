...
public class SCPBlockKeycardSlotOmni extends SCPBlockKeycardSlot
{	
	...
	public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
	{
		...
		if (par1World.isRemote)
		{
			...
			if(itemstack != null && (itemstack.itemID == mod_SCP.OmniKeycard.itemID)) 
			{
				int k = 8 - (i & 8);
			...
			}else
			{
				if(itemstack != null && (itemstack.itemID == mod_SCP.Wrench.itemID || itemstack.itemID == mod_SCP.GodlyWrench.itemID)) {
					par1World.setBlockMetadataWithNotify(par2, par3, par4, mod_SCP.KeycardSlot.blockID, j);
					itemstack.damageItem(1, par5EntityPlayer);
					if(par1World.isRemote)
					{
						par5EntityPlayer.addChatMessage("Keycard Level: 1");
					}
					if(!par1World.isRemote)
					{
						par5EntityPlayer.addChatMessage("Keycard Level: 1");
				...
				else
				{
					if(par1World.isRemote)
					{
						par5EntityPlayer.addChatMessage("You need an Omni Keycard to activate.");
					}
					if(!par1World.isRemote)
					{
						par5EntityPlayer.addChatMessage("You need an Omni Keycard to activate.");
