...
public abstract class SCPBlockFluid extends BlockFluid
{
    public Icon[] theIcon;

    protected SCPBlockFluid(int par1, Material par2Material)
    {
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        //TODO
        if (this.blockMaterial == SCPMaterial.SCP354 && par1World.getBlockMaterial(par2, par3 + 1, par4) == Material.air && !par1World.isBlockOpaqueCube(par2, par3 + 1, par4))
        {
            if (par5Random.nextInt(100) == 0)
            {
                ...
                par1World.playSound(d5, d7, d6, "mob.ghast.scream", 0.2F + par5Random.nextFloat() * 0.2F, 0.9F + par5Random.nextFloat() * 0.15F, false);
            }
        }
    ...
    public static double getFlowDirection(IBlockAccess par0IBlockAccess, int par1, int par2, int par3, Material par4Material)
    {
        ...
        //TODO
        if (par4Material == SCPMaterial.SCP354)
        {
            vec3 = ((SCPBlockFluid)mod_SCP.SCP354Flowing).getFlowVector(par0IBlockAccess, par1, par2, par3);
        }
    ...
    private void checkForHarden(World par1World, int par2, int par3, int par4)
    {
        if (par1World.getBlockId(par2, par3, par4) == this.blockID)
        {
            ...
            //TODO
            if (this.blockMaterial == SCPMaterial.SCP354)
            {
                boolean var5 = false;

                if (var5 || par1World.getBlockMaterial(par2, par3, par4 - 1) == Material.water || par1World.getBlockMaterial(par2, par3, par4 - 1) == Material.lava)
                {
                    var5 = true;
                }
                ...
                if (var5 || par1World.getBlockMaterial(par2 - 1, par3, par4) == Material.water || par1World.getBlockMaterial(par2 - 1, par3, par4) == Material.lava)
                {
                    var5 = true;
                }
                ...
                if (var5 || par1World.getBlockMaterial(par2, par3 + 1, par4) == Material.water || par1World.getBlockMaterial(par2, par3 + 1, par4) == Material.lava)
                {
                    var5 = true;
                }
                ...
                if (var5)
                {
                    ...
                    float var1 = 0.3F;
                    par1World.createExplosion((Entity)null, par2, par3, par4, var1, true);
                    if (var6 == 0)
                    {
                        par1World.setBlock(par2, par3, par4, mod_SCP.BloodBlock.blockID);
                    }
                    else if (var6 <= 4)
                    {
                        par1World.setBlock(par2, par3, par4, mod_SCP.BloodBlock.blockID);
                    ...
                    this.triggerSCP354MixEffects(par1World, par2, par3, par4);
                }
            }
        }
    }
        ...
        //TODO
        protected void triggerSCP354MixEffects(World par1World, int par2, int par3, int par4)
        {
            par1World.playSoundEffect((double)((float)par2 + 0.5F), (double)((float)par3 + 0.5F), (double)((float)par4 + 0.5F), "random.explode", 0.5F, 2.6F + (par1World.rand.nextFloat() - par1World.rand.nextFloat()) * 0.8F);

            for (int var5 = 0; var5 < 8; ++var5)
            {
                par1World.spawnParticle("explosion", (double)par2 + Math.random(), (double)par3 + 1.2D, (double)par4 + Math.random(), 0.0D, 0.0D, 0.0D);
            }
        }
  ...
  //  @SideOnly(Side.CLIENT)
    ...
    //{
     //   if(par0Str == "SCP354") return mod_SCP.SCP354Still.theIcon[0]; 
      //  if(par0Str == "SCP354_flow") return mod_SCP.SCP354Flowing.theIcon[1];
    //}
    //FIXME
}
