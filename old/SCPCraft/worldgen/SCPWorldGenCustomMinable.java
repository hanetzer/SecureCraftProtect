...
public class SCPWorldGenCustomMinable extends WorldGenerator
{
            ...
            for(int k2 = i1; k2 <= l1; k2++)
            {
                ...
                for(int l2 = j1; l2 <= i2; l2++)
                {
                    ...
                    for(int i3 = k1; i3 <= j2; i3++)
                    {
                        ...
                        if(d12 * d12 + d13 * d13 + d14 * d14 < 1.0D && world.getBlockId(k2, l2, i3) == baseBlock)
                        {
    ...
    public boolean generate(World world, Random random, int i, int j, int k, int m)
    {
        float f = random.nextFloat() * 3.141593F;
        double d = (float)(i + 8) + (MathHelper.sin(f) * (float)numberOfBlocks) / 8F;
        double d1 = (float)(i + 8) - (MathHelper.sin(f) * (float)numberOfBlocks) / 8F;
        ...
        for(int l = 0; l <= numberOfBlocks; l++)
        {
            ...
            for(int k2 = i1; k2 <= l1; k2++)
            {
                ...
                for(int l2 = j1; l2 <= i2; l2++)
                {
                    ...
                    {
                        double d14 = (((double)i3 + 0.5D) - d8) / (d10 / 2D);
                        if(d12 * d12 + d13 * d13 + d14 * d14 < 1.0D && world.getBlockId(k2, l2, i3) == baseBlock && world.getBlockId(k2, l2, i3) == m)
                        {
                            world.setBlock(k2, l2, i3, minableBlockId);
                        }
                    }
